...
console.log(
  inventory[lastCar]["car_make"] + " " + inventory[lastCar]["car_model"]
);
...
function compareModels(a, b) {
  ...
  console.log(inventory.sort(compareModels));
...
function oldSort() {
    for (let i = 0; i < inventory.length; i++) {
        if (inventory[i]["car_year"] < 2000) oldCars.push(inventory[i]["car_year"]);
    }
}
oldSort();
...
function findCars(obj) {
    if (obj < 2000) {
      oldCars.push(obj);
    }
  }
  carYears.filter(findCars);
  console.log(oldCars.length);
// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];
function findGerman(obj) {
  if (obj["car_make"] == "BMW") {
    BMWAndAudi.push(obj);
  } else if (obj["car_make"] == "Audi") {
    BMWAndAudi.push(obj);
  }
}
inventory.filter(findGerman);
